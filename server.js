document.addEventListener('DOMContentLoaded', () => {
    ...
    // State variables to solve the YouTube player race condition
    ...
    function onPlayerReady(event) {
        ...
        if (pendingYouTubeTrack) {
            console.log('Playing pending YouTube track.');
            playYouTubeTrack(pendingYouTubeTrack);
    ...
    volumeSlider.addEventListener('input', (e) => {
        ...
        if (youtubePlayer && youtubePlayer.setVolume) youtubePlayer.setVolume(volume);
    });
    ...
    // --- HELPER FUNCTION TO PLAY YOUTUBE TRACKS ---
    function playYouTubeTrack(nowPlaying) {
        if (!nowPlaying || !nowPlaying.track) return;
        const track = nowPlaying.track;
    ...
    socket.on('newSongPlaying', (nowPlaying) => {
        ...
        if (track.source === 'spotify') {
            ...
            if (isYouTubePlayerReady) {
                playYouTubeTrack(nowPlaying);
            } else {
                console.log('YouTube player not ready, queuing track...');
                pendingYouTubeTrack = nowPlaying;
            }
        }
    });
